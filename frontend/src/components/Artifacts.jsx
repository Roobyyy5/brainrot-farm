...
export default function Artifacts() {
  ...
  // Compute active sets on client side
  ...
  return (
    <div className="artifacts-panel">
      <div className="artifacts-header">⚙️ Artifacts</div>

      {equippedKeys.length >= 2 && (
        <div className="artifacts-sets">
          <div className="artifacts-sets-title">Active Sets</div>
          {Object.entries({
      ...
      <div className="artifacts-tabs">
        <button className={tab === 'equipped' ? 'active' : ''} onClick={() => setTab('equipped')}>Equipped</button>
              ...
              <div key={slot} className="artifact-slot">
                ...
                {art ? (
                  <div className="artifact-card" style={{ borderColor: RARITY_COLOR[art.rarity] }}>
                    ...
                    <button className="artifact-unequip-btn" onClick={() => unequip(slot)} disabled={loading}>Unequip</button>
                  </div>
                ) : (
                  <div className="artifact-slot-empty">Empty</div>
                )}
              </div>
            );
          })}
      ...
      {tab === 'inventory' && (
        <div className="artifacts-inventory">
          {Object.values(grouped).length === 0 && (
            <div className="artifacts-empty">No artifacts yet. Get them from boss drops, loot boxes, and prestige rewards!</div>
          )}
            ...
            <div key={`${g.key}:${g.rarity}`} className="artifact-inv-card" style={{ borderColor: RARITY_COLOR[g.rarity] }}>
              ...
              <div className="artifact-inv-actions">
                {!data.equipped?.[g.slot] && (
                  <button className="artifact-equip-btn" onClick={() => equip(g.ids[0])} disabled={loading}>Equip</button>
                )}
