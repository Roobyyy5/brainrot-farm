...
router.post('/equip', asyncHandler(async (req, res) => {
  ...
  const r = await pool.query('SELECT skins_unlocked, prestige FROM tapper_profiles WHERE telegram_id = $1', [telegramId]);
  ...
  await pool.query('UPDATE tapper_profiles SET selected_skin = $1 WHERE telegram_id = $2', [skinKey, telegramId]);
